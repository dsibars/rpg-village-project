...
test('ExploreView DOM Refactor', async (t) => {
    ...
    await t.test('mounts successfully and renders expedition list', () => {
        ...
        const cards = listContainer.querySelectorAll('.expedition-card');
    ...
    await t.test('selecting expedition updates detail pane', () => {
        document.body.innerHTML = html;
        const rootNode = document.body.querySelector('#explore-view');
        ...
        const detailPane = document.body.querySelector('#expedition-detail-content');
        assert.ok(detailPane);

        const content = detailPane.querySelector('.expedition-profile');
        assert.ok(content);

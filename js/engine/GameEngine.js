...
import { DayResolutionService } from './shared/services/DayResolutionService.js';
export class GameEngine {
    constructor() {
        ...
        // Daily tick orchestration (kept out of the facade)
        this.dayResolutionService = new DayResolutionService({
            villageService: this.villageService,
            heroService: this.heroService,
            expeditionService: this.expeditionService,
            calendarService: this.calendarService,
            academyService: this.academyService,
            missionSeedService: this.missionSeedService,
            dailyObjectivesService: this.dailyObjectivesService,
            dailyHeroActionsService: this.dailyHeroActionsService,
            villageEventsService: this.villageEventsService,
            bookService: this.bookService,
            chronicleService: this.chronicleService,
            unlockService: this.unlockService,
            presentationService: this.presentationService,
            i18n: this.i18n,
            hooks: {
                reportTutorialEvent: (p) => this.reportTutorialEvent(p),
                processPresentationTriggers: (p) => this._processPresentationTriggers(p),
                persistPresentationState: () => this._persistPresentationState(),
                evaluateChapterMilestones: (n) => this._evaluateChapterMilestones(n),
                buildUnlockState: () => this._buildUnlockState()
            }
        });
    ...
    // --- Time & Construction ---
    nextDay() {
        return this.dayResolutionService.resolve();
    }
                        ...
                        blocks: [
                            {
                                ...
                                values: {
                                    ...
                                    // combatLog.enemies is already a name array (strings)
                                    enemies: combatLog.enemies?.filter(Boolean).join(', ')

...
usage() {
    cat <<'EOF'
Usage:
  memory.sh add <topic> <tags> <summary> <details> [files]
  memory.sh search <query> [limit] [hash_filter]
...
Examples:
  memory.sh add magic_circle_v2 "ux,bug" "Tier dial bug" "Value placeholder leaks" "MagicCircleViewV2.js"
  memory.sh search "tier dial" 5
  memory.sh pm-search pm_combat current
  memory.sh pm-compare pm_combat 25f4d647277d5c156d60dacdd666fa66 1669e19c9012e2261c2800a773ca40fa
...
cmd_recent() {
    local limit="${1:-10}"
    sqlite3 "$DB" -header -json "SELECT rowid, topic, tags, summary, details, timestamp, files, docs_hash FROM memories ORDER BY timestamp DESC LIMIT $limit;"
}
